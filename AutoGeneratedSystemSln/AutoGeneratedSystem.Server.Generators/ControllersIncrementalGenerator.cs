                ...
                foreach (var singleAttributeList in singleClassDeclarationSyntax.AttributeLists)
                {
                    foreach (var singleAttribute in singleAttributeList.Attributes)
                    {
                        ...
                        if (identifierText == "ControllerOfEntity")
                        {
                            ...
                            var memberAccessExpressionSyntax = entityNameArgument.Expression as MemberAccessExpressionSyntax;
                            ...
                            stringBuilder.AppendLine($"public async Task<IActionResult> Delete{entityName}({primaryKeyTypeValue} entityId, [FromServices] {entityName}Service {entityName}Service, CancellationToken cancellationToken)");
                            stringBuilder.AppendLine("{");
                            ...
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("[HttpGet(\"[action]\")]");
                            stringBuilder.AppendLine($"public async Task<{entityName}Model> Get{entityName}ById([FromQuery]{primaryKeyTypeValue} entityId, [FromServices] {entityName}Service {entityName}Service, [FromServices] IMapper mapper, CancellationToken cancellationToken)");
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"var entity = await {entityName}Service.Get{entityName}ByIdAsync(entityId, cancellationToken);");
