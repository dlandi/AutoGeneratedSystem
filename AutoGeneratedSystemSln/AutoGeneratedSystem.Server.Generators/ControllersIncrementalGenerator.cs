                ...
                foreach (var singleAttributeList in singleClassDeclarationSyntax.AttributeLists)
                {
                    foreach (var singleAttribute in singleAttributeList.Attributes)
                    {
                        ...
                        if (identifierText == "ControllerOfEntity")
                        {
                            ...
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("[HttpGet(\"[action]\")]");
                            stringBuilder.AppendLine($"public async Task<{entityName}Model> Get{entityName}ById([FromQuery]{primaryKeyTypeValue} primarykeyValue, [FromServices] {entityName}Service {entityName}Service, [FromServices] IMapper mapper, CancellationToken cancellationToken)");
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"var entity = await {entityName}Service.Get{entityName}ByIdAsync(primarykeyValue, cancellationToken);");
