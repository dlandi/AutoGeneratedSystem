                ...
                foreach (var singleAttributeList in singleClassDeclarationSyntax.AttributeLists)
                {
                    foreach (var singleAttribute in singleAttributeList.Attributes)
                    {
                        ...
                        if (identifierText == "ClientServiceOfEntity")
                        {
                            ...
                            stringBuilder.AppendLine($"var result = await anonymousHttpClient.GetFromJsonAsync<{entityName}Model[]>(requestUrl);");
                            stringBuilder.AppendLine("return result;");
                            stringBuilder.AppendLine("}");
                            ...
                            stringBuilder.AppendLine("var anonymousHttpClient = _httpClientService.CreateAnonymousClient();");
                            stringBuilder.AppendLine("var response = await anonymousHttpClient.DeleteAsync(requestUrl);");
                            stringBuilder.AppendLine("response.EnsureSuccessStatusCode();");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
