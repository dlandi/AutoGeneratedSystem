                ...
                foreach (var singleAttributeList in singleClassDeclarationSyntax.AttributeLists)
                {
                    foreach (var singleAttribute in singleAttributeList.Attributes)
                    {
                        ...
                        if (identifierText == "ServiceOfEntity")
                        {
                            ...
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"return _autogeneratedsystemDatabaseContext.{entityName}.AsTracking();");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("else");
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"return _autogeneratedsystemDatabaseContext.{entityName}.AsNoTracking();");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
                            ...
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"var entity = await this._autogeneratedsystemDatabaseContext.{entityName}" +
                            ...
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine($"_autogeneratedsystemDatabaseContext.{entityName}.Remove(entity);");
                            stringBuilder.AppendLine($"await _autogeneratedsystemDatabaseContext.SaveChangesAsync(cancellationToken:cancellationToken);");
                            stringBuilder.AppendLine("}");
                            ...
                            stringBuilder.AppendLine($"if (trackEntities)");
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"return await this._autogeneratedsystemDatabaseContext.{entityName}.AsTracking().Where(p=>p.{entityName}Id == {entityName}Id).SingleOrDefaultAsync(cancellationToken:cancellationToken);");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("else");
                            stringBuilder.AppendLine("{");
                            stringBuilder.AppendLine($"return await this._autogeneratedsystemDatabaseContext.{entityName}.AsNoTracking().Where(p=>p.{entityName}Id == {entityName}Id).SingleOrDefaultAsync(cancellationToken:cancellationToken);");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
