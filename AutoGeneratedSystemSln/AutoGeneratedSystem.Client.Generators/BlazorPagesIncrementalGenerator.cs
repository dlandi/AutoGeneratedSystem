...
namespace AutoGeneratedSystem.Client.Generators
{
    ...
    public class BlazorPagesIncrementalGenerator : IIncrementalGenerator
    {
            ...
            string entityName, string assemblyNameFirstPart)
        {
            ...
            razorCreatePage.AppendLine($"<h3>@(\"{entityName}\".Humanize().Titleize())</h3>");
            razorCreatePage.AppendLine("<button type=\"button\" class=\"btn btn - info rounded - circle\"");
            razorCreatePage.AppendLine($"@onclick=\"@(() => NavigationHelper.NavigateToListPageForEntity(this.NavigationManager,\"{entityName}\"))\"");
            razorCreatePage.AppendLine("data-bs-toggle=\"tooltip\" data-bs-placement=\"top\"");
            razorCreatePage.AppendLine($"title=\"@(\"List {entityName}\".Humanize().Titleize())\">");
            razorCreatePage.AppendLine("<i class=\"bi bi-list\" style=\"font-size:large\"></i>");
            razorCreatePage.AppendLine("</button>");
            ...
            List<JunctionItemInfo> junctionTablesInfo = new();
            foreach (IPropertySymbol singleProperty in modelProperties)
            {
                bool isJunctionTableProperty = false;
                ...
                if (propertyAttributes.Length > 0)
                {
                    ...
                    {
                        isJunctionTableProperty = true;
                        foreach (var singleJunctionTableAttribute in junctionTableAttributes)
                        {
                            var junctionTableModelName =
                                singleJunctionTableAttribute.ConstructorArguments[0].Value!.ToString();
                            var displayTextFieldName =
                                singleJunctionTableAttribute.ConstructorArguments[1].Value!.ToString();
                            var valueFieldName =
                                singleJunctionTableAttribute.ConstructorArguments[2].Value!.ToString();
                            junctionTablesInfo.Add(new JunctionItemInfo()
                            {
                                DisplayTextFieldName = displayTextFieldName,
                                EntityName = junctionTableModelName,
                                ValueFieldName = valueFieldName
                            });
                        }
                    }
                }
                if (!isJunctionTableProperty)
                {
                    ...
                    switch (propertyType.ToString())
                    {
                        ...
                        case "bool":
                            razorCreatePage.AppendLine("<div class=\"mb-3\">");
                            razorCreatePage.AppendLine($"<label class=\"form-check-label\">@(nameof(Model.{singleProperty.Name}).Humanize().Titleize())</label>");
                        ...
                        case "short":
                            razorCreatePage.AppendLine("<div class=\"mb-3\">");
                            razorCreatePage.AppendLine($"<label class=\"form-label\">@(nameof(Model.{singleProperty.Name}).Humanize().Titleize())</label>");
            ...
            if (junctionTablesInfo.Any())
            {
                foreach (var singleJunctionTableItem in junctionTablesInfo)
                {
                    razorCreatePage.AppendLine($"@if (this.{singleJunctionTableItem.EntityName}ModelItems != null)");
                    razorCreatePage.AppendLine("{");
                    razorCreatePage.AppendLine("<div class=\"mb-3\">");
                    razorCreatePage.AppendLine($"<label class=\"form-label\">@(nameof(Model.{singleJunctionTableItem.EntityName}Id).Humanize().Titleize())</label>");
                    razorCreatePage.AppendLine($"<InputSelect class=\"form-control\" @bind-Value=\"Model.{singleJunctionTableItem.EntityName}Id\">");
                    razorCreatePage.AppendLine($"<option value=\"\">Select {singleJunctionTableItem.EntityName}...</option>");
                    razorCreatePage.AppendLine($"@foreach (var single{singleJunctionTableItem.EntityName}Item in this.{singleJunctionTableItem.EntityName}ModelItems)");
                    razorCreatePage.AppendLine("{");
                    razorCreatePage.AppendLine($"<option value=\"@single{singleJunctionTableItem.EntityName}Item.{singleJunctionTableItem.ValueFieldName}\">@single{singleJunctionTableItem.EntityName}Item.{singleJunctionTableItem.DisplayTextFieldName}</option>");
                    razorCreatePage.AppendLine("}");
                    razorCreatePage.AppendLine("</InputSelect>");
                    razorCreatePage.AppendLine("</div>");
                    razorCreatePage.AppendLine("}");
                }
            }
            ...
            pageCodeBehind.AppendLine("[Inject] private NavigationManager NavigationManager { get; set; }");
            ...
            if (junctionTablesInfo.Any())
            {
                foreach (var singleJunctionTableItem in junctionTablesInfo)
                {
                    pageCodeBehind.AppendLine($"private long {singleJunctionTableItem.EntityName}Id {{ get; set; }}");
                    pageCodeBehind.AppendLine($"private {assemblyNameFirstPart}.Models.{singleJunctionTableItem.EntityName}.{singleJunctionTableItem.EntityName}Model[] " +
                        $"{singleJunctionTableItem.EntityName}ModelItems{{ get; set; }}");
                    pageCodeBehind.AppendLine($"[Inject] private {singleJunctionTableItem.EntityName}ClientService " +
                        $"{singleJunctionTableItem.EntityName}ClientService {{ get; set; }}");
                }
            }
            ...
            foreach (var singleJunctionTableItem in junctionTablesInfo)
            {
                pageCodeBehind.AppendLine($"this.{singleJunctionTableItem.EntityName}ModelItems = " +
                    $"await {singleJunctionTableItem.EntityName}ClientService.GetAll{singleJunctionTableItem.EntityName}Async();");
            }
            pageCodeBehind.AppendLine("}");
            ...
            pageCodeBehind.AppendLine($"this.NavigationManager.NavigateTo(\"/{entityName}/List\");");
            ...
            string assemblyNameFirstPart)
        {
            ...
            razorPage.AppendLine($"<h3>@(\"{entityName}\".Pluralize().Humanize().Titleize())</h3>");
            ...
            razorPage.AppendLine("<div class=\"mb-3\">");
            razorPage.AppendLine("<button type=\"button\" class=\"btn btn-info rounded-circle\"");
            razorPage.AppendLine($"@onclick=\"@(() => NavigationHelper.NavigateToCreatePageForEntity(this.NavigationManager,\"{entityName}\"))\"");
            razorPage.AppendLine("data-bs-toggle=\"tooltip\" data-bs-placement=\"top\"");
            razorPage.AppendLine($"title=\"Create {entityName}\">");
            razorPage.AppendLine("<i class=\"bi bi-plus-circle-fill\" style=\"font-size:large\"></i>");
            razorPage.AppendLine("</button>");
            razorPage.AppendLine("</div>");
            razorPage.AppendLine("<div class=\"mb-3\">");
            ...
            {
                razorPage.AppendLine("<li class=\"list-group-item\">");
                razorPage.AppendLine($"<label class=\"form-label\"><strong>@(nameof(single{entityName}.{singleProperty.Name}).Humanize().Titleize())</strong></label>");
                var propertyAttributes = singleProperty.GetAttributes();
                if (propertyAttributes.Any(p => p.AttributeClass != null &&
                p.AttributeClass!.MetadataName == "ImageUrlAttribute"))
                {
                    var imageUrlAttribute = propertyAttributes
                        .Where(p => p.AttributeClass != null && p.AttributeClass.MetadataName == "ImageUrlAttribute")
                        .SingleOrDefault();
                    if (imageUrlAttribute != null)
                    {
                        razorPage.AppendLine($"<a href=\"@single{entityName}.{singleProperty.Name}\" target=\"_blank\"><img class=\"img-fluid\" src=\"@single{entityName}.{singleProperty.Name}\" /></a>");
                    }
                }
                else
                    razorPage.AppendLine($"<span>@single{entityName}.{singleProperty.Name}</span>");
